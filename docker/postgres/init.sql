...
CREATE INDEX IF NOT EXISTS idx_usuarios_sucursal_activo ON usuarios(sucursal_id, activo);
...
CREATE INDEX IF NOT EXISTS idx_productos_sucursal_activo ON productos(sucursal_id, activo);
CREATE INDEX IF NOT EXISTS idx_productos_sku ON productos(sku);

-- Columna tipo (VENTA / PRODUCCION)
ALTER TABLE productos ADD COLUMN IF NOT EXISTS tipo VARCHAR(20) NOT NULL DEFAULT 'VENTA';

-- Columna medida y presentaciones
...
CREATE INDEX IF NOT EXISTS idx_ventas_sucursal_estado ON ventas(sucursal_id, estado);
CREATE INDEX IF NOT EXISTS idx_ventas_sucursal_fecha ON ventas(sucursal_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_cierres_sucursal_fecha ON cierres_caja(sucursal_id, fecha_cierre);
...
CREATE INDEX IF NOT EXISTS idx_bitacora_sucursal_fecha ON bitacora_actividad(sucursal_id, created_at);
CREATE INDEX IF NOT EXISTS idx_bitacora_usuario ON bitacora_actividad(usuario_id);
CREATE INDEX IF NOT EXISTS idx_bitacora_modulo ON bitacora_actividad(modulo);
CREATE INDEX IF NOT EXISTS idx_bitacora_accion ON bitacora_actividad(accion);
CREATE INDEX IF NOT EXISTS idx_bitacora_entidad ON bitacora_actividad(entidad, entidad_id);

-- ============================================================
-- 8. SEED DATA
-- ============================================================

-- Sucursal principal (solo si no existe)
INSERT INTO sucursales (id, nombre, direccion, telefono)
SELECT gen_random_uuid(), 'La Pale Principal', 'Calle Principal #123', '555-0100'
WHERE NOT EXISTS (SELECT 1 FROM sucursales WHERE nombre = 'La Pale Principal');

-- Roles con IDs fijos para integrar con el frontend
