...
// ConfirmarVenta ejecuta todo en una transacción:
// 1. Valida stock de cada producto
// 2. Inserta venta + detalle + pago
// 3. Descuenta inventario + registra movimientos
func (r *Repository) ConfirmarVenta(v *Venta, detalle []DetalleVenta, pago Pago, productos map[string]productoStock) error {
	...
	err = tx.QueryRow(ctx, fmt.Sprintf(`
		...
		VALUES ($1,$2,%s,$3,'CERRADA',$4,$5,$6)
		RETURNING id, ticket_numero, created_at, updated_at
	`, autorizadoPor),
		v.SucursalID, v.VendedorID, v.Tipo, v.Subtotal, v.Total, v.Notas,
	...
	if pago.Metodo == MetodoTarjeta {
		estadoPago = EstadoPagoPendiente // se confirma cuando llega webhook de MP
	}
